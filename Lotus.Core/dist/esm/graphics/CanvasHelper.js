...
export class CanvasHelper {
     ...
     */
    static clipCanvasRounded(source, borderRadius, targetCanvas) {
     ...
     */
    static removeImageBackground(image, threshold = 0.1) {
        ...
        while (stack.length > 0) {
            ...
            const isBackground = bgColors.some(bg => {
                const distance = Math.sqrt((r - bg.r) ** 2 +
                    (g - bg.g) ** 2 +
                    (b - bg.b) ** 2);
     ...
     * @param threshold - Чувствительность (0.0 - 1.0). Оптимально 0.1 - 0.15.
     */
    static removeBackground(context, threshold = 0.1) {
     ...
     * @param threshold - Чувствительность (0.0 - 1.0). Оптимально 0.1 - 0.15.
     */
    static removeSpriteBackground(image, threshold = 0.1) {
        const canvas = document.createElement('canvas');
        canvas.width = image.width;
        canvas.height = image.height;
        const ctx = canvas.getContext('2d', { willReadFrequently: true });
     ...
     * @param threshold - Чувствительность (0.0 - 1.0). Оптимально 0.1 - 0.15.
     */
    static removeSpriteBackgroundFromContext(context, threshold = 0.1) {
        const imageData = context.getImageData(0, 0, context.canvas.width, context.canvas.height);
        const { data, width, height } = imageData;
        const maxDist = threshold * 441.67;
        const visited = new Uint8Array(width * height);
     ...
     */
    static cleanupIsolatedPixels(canvas, neighborThreshold = 2) {
        const ctx = canvas.getContext('2d', { willReadFrequently: true });
        if (!ctx)
            return canvas;
        const imageData = ctx.getImageData(0, 0, canvas.width, canvas.height);
     ...
     */
    static smoothCanvasEdges(canvas, smoothness = 1, cutoff = 128) {
        const ctx = canvas.getContext('2d', { willReadFrequently: true });
        if (!ctx)
            return canvas;
     ...
     * @returns Обработанный холст с четким сглаженным краем.
     */
    static smoothCanvasEdgesFromContext(context, smoothness = 1, cutoff = 128) {
        // 1. Применяем легкий блюр только для визуального смягчения альфы
        // Мы используем фильтр контекста, это быстрее ручного перебора
        context.save();
