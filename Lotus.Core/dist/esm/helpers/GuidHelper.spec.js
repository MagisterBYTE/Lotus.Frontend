...
describe('GuidHelper', () => {
    ...
    describe('createGuid', () => {
        it('should generate a GUID string', () => {
            const uuid = GuidHelper.createGuid();
            expect(typeof uuid).toBe('string');
            expect(uuid).toHaveLength(36); // UUID имеет фиксированную длину 36 символов
        });
        it('should follow the GUID format', () => {
            const uuid = GuidHelper.createGuid();
            expect(GuidHelper.TGuidRegex.test(uuid)).toBe(true); // Проверяем, соответствует ли UUID шаблону
        });
        ...
        it('should generate a unique GUID each time', () => {
            ...
            const guid2 = GuidHelper.createGuid();
            expect(guid1).not.toEqual(guid2);
        });
        it('should generate unique GUID', () => {
            const uuidSet = new Set();
            for (let i = 0; i < 1000; i++) {
                uuidSet.add(GuidHelper.createGuid());
            }
