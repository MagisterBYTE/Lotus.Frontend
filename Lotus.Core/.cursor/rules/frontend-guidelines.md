...
description: Это требования по стилю и базовой архитектуре написания кода для frontend приложений
---
...
- **Функциональные типы**: Все типы, объявляющие сигнатуру функции (function types), **обязательно** должны иметь суффикс `Delegate` или `Function`.
    ...
    - *Пример:* `type RenderFunction = (item: unknown, context?: unknown) => any`
...
- **Группировка и порядок регионов**: Все члены класса должны быть обернуты в `#region` / `#endregion`. Регионы должны следовать строго в указанном порядке:
    1. `Const` (для `static readonly` полей)
    2. `Instance` (только для классов, реализующих паттерн ClassInstance)
    3. `Static fields`
    4. `Static properties`
    5. `Fields`
    6. `Properties`
    7. `Constructors`
    8. `Methods` (для обычных методов класса)
    9. `[Имя интерфейса] methods` (для реализации методов конкретного интерфейса)
