...
const PRODUCT_CATEGORIES = ['electronics', 'books', 'clothing'] as const;
...
describe('TypedRoute', () => {
  ...
  describe('Parameter constraints', () => {
    const constrainedRoute = TypedRoute.createWithConstraints(
      '/admin/:role/:action',
      {
        role: USER_ROLES,
        action: ['view', 'edit', 'delete'] as const
      } as const);
  ...
  describe('Combined parameters and query', () => {
    const userProfileRoute = new TypedRoute(
      '/user/:userId/profile/:tab',
      true,
      { tab: ['settings', 'security', 'notifications'] as const }
    );

    test('should combine path and query parameters', () => {
      const result = userProfileRoute.withParamsAndQuery(
        { userId: '123', tab: 'settings' },
        { modal: 'true', section: 'password' }
      );
  ...
  describe('Utility methods', () => {
    const constrainedRoute = new TypedRoute(
      '/api/:version/:resource',
      false,
      {
        version: ['v1', 'v2'] as const,
        resource: ['users', 'products'] as const
      }
    );
  ...
  describe('TypeScript type safety', () => {
    ...
    test('should provide autocomplete for constrained parameters', () => {
      ...
      const route = new TypedRoute(
        '/admin/:action',
        true,
        { action: ['create', 'read', 'update', 'delete'] as const }
      );
  ...
  describe('Performance', () => {
    test('should handle large number of parameters efficiently', () => {
      const complexRoute = new TypedRoute(
        '/api/:p1/:p2/:p3/:p4/:p5',
        false,
        {
          p1: ['a', 'b'] as const,
