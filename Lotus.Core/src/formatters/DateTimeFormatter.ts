...
export abstract class DateTimeFormatter
{
   ...
   */
  public static isDateTimeFormatSupported(): boolean
  {
   ...
   * ```
   */
  public static date(date: Date, locale: string = navigator.language, options?: Intl.DateTimeFormatOptions): string
  {
   ...
   * ```
   */
  public static dateTime(date: Date, locale: string = navigator.language, options?: Intl.DateTimeFormatOptions): string
  {
   ...
   * ```
   */
  public static time(date: Date, locale: string = navigator.language, options?: Intl.DateTimeFormatOptions): string
  {
   ...
   * @returns {boolean} true если API поддерживается
   */
  private static isDurationFormatSupported(): boolean
  {
   ...
   * ```
   */
  public static formatDurationObject(duration: Intl.DurationInput, locale: string = navigator.language, options?: Intl.DurationFormatOptions): string
  {
    ...
    if (this.isDurationFormatSupported())
    {
      try
      {
        const defaultOptions: Intl.DurationFormatOptions = {
          style: 'long',
          yearsDisplay: 'auto',
          monthsDisplay: 'auto',
          weeksDisplay: 'auto',
   ...
   * @returns {string} Отформатированная строка продолжительности
   */
  private static formatDurationObjectFallback(duration: Intl.DurationInput, locale: string, options?: Intl.DurationFormatOptions): string
  {
    ...
    if (duration.years) totalSeconds += duration.years * 31536000; // 365 дней
    if (duration.months) totalSeconds += duration.months * 2592000; // 30 дней
    if (duration.weeks) totalSeconds += duration.weeks * 604800; // 7 дней
    if (duration.days) totalSeconds += duration.days * 86400; // 24 часа
    if (duration.hours) totalSeconds += duration.hours * 3600; // 60 минут
    if (duration.minutes) totalSeconds += duration.minutes * 60; // 60 секунд
  ...
  private static calculateMaxUnits(duration: Intl.DurationInput, options?: Intl.DurationFormatOptions): number
  {
    ...
    let unitCount = 0;
    if (duration.years && (options?.yearsDisplay === 'always' || duration.years > 0)) unitCount++;
    if (duration.months && (options?.monthsDisplay === 'always' || duration.months > 0)) unitCount++;
    if (duration.weeks && (options?.weeksDisplay === 'always' || duration.weeks > 0)) unitCount++;
    if (duration.days && (options?.daysDisplay === 'always' || duration.days > 0)) unitCount++;
    if (duration.hours && (options?.hoursDisplay === 'always' || duration.hours > 0)) unitCount++;
    if (duration.minutes && (options?.minutesDisplay === 'always' || duration.minutes > 0)) unitCount++;
    if (duration.seconds && (options?.secondsDisplay === 'always' || duration.seconds > 0)) unitCount++;
    if (duration.milliseconds && (options?.millisecondsDisplay === 'always' || duration.milliseconds > 0)) unitCount++;
   ...
   * ```
   */
  public static formatDurationWithIntl(seconds: number, locale: string = navigator.language, options?: Intl.DurationFormatOptions): string
  {
   ...
   */
  private static secondsToDurationObject(seconds: number): Intl.DurationInput
  {
   ...
   * ```
   */
  public static formatDurationDigital(
    seconds: number,
   ...
   * ```
   */
  public static formatDurationToParts(seconds: number, locale: string = navigator.language, options?: Intl.DurationFormatOptions): Intl.DurationFormatPart[]
  {
   ...
   */
  private static formatDurationToPartsFallback(seconds: number, locale: string, options?: Intl.DurationFormatOptions): Intl.DurationFormatPart[]
  {
  ...
  private static guessUnitFromToken(token: string, locale: string): Intl.DurationFormatUnitSingular
  {
    ...
    {
      if (lowerToken.includes('год') || lowerToken.includes('лет')) return 'year';
      if (lowerToken.includes('месяц')) return 'month';
      if (lowerToken.includes('недел')) return 'week';
      if (lowerToken.includes('дн') || lowerToken.includes('ден')) return 'day';
      if (lowerToken.includes('час')) return 'hour';
      if (lowerToken.includes('минут')) return 'minute';
      if (lowerToken.includes('секунд')) return 'second';
    }

    // Для английского языка
    if (locale.startsWith('en'))
    {
      if (lowerToken.includes('year')) return 'year';
      if (lowerToken.includes('month')) return 'month';
      if (lowerToken.includes('week')) return 'week';
      if (lowerToken.includes('day')) return 'day';
      if (lowerToken.includes('hour')) return 'hour';
      if (lowerToken.includes('minute') || lowerToken.includes('min')) return 'minute';
      if (lowerToken.includes('second') || lowerToken.includes('sec')) return 'second';
    }
   ...
   * @returns {boolean} true если API поддерживается
   */
  public static isRelativeTimeFormatSupported(): boolean
  {
   ...
   * ```
   */
  public static formatRelativeValue(
    value: number,
  ...
  ): string
  {
    ...
    const availableUnits: Array<{
      ...
      minValue?: number; // Минимальное значение для отображения
    }> = [
   ...
   * ```
   */
  public static formatRelativeOfDate(targetDate: Date, locale: string = navigator.language, options?: Intl.RelativeTimeFormatOptions): string
  {
    const now = new Date();
   ...
   * ```
   */
  public static getWeekdays(locale: string = navigator.language, format: 'long' | 'short' | 'narrow' = 'long'): string[]
  {
   ...
   * ```
   */
  public static getMonths(locale: string = navigator.language, format: 'long' | 'short' | 'narrow' = 'long'): string[]
  {
    const formatter = new Intl.DateTimeFormat(locale, {
      month: format,
      timeZone: 'UTC'
   ...
   * ```
   */
  public static toISOLocal(date: Date): string
  {
