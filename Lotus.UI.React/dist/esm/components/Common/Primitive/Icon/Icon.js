...
export const Icon = memo((props) => {
    ...
    if (typeof icon === 'string' && (ImageHelper.isDataURL(icon) || BrowserHelper.isAbsoluteUrl(icon))) {
        return (_jsx("img", { alt: "icon", crossOrigin: crossOrigin, height: sizePx, loading: "lazy" // Оптимизация для галерей: не грузить то, что вне экрана
            , src: icon, style: iconStyle, width: sizePx }));
    }
    ...
    if ((typeof icon === 'number' || typeof icon === 'string') && imageDatabase) {
        ...
        return (_jsx("img", { alt: iconData.name || 'database-icon', crossOrigin: crossOrigin, height: sizePx, loading: "lazy", src: iconData.source, style: iconStyle, width: sizePx }));
    }
