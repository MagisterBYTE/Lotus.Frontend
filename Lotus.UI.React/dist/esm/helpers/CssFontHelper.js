...
import { instanceOfLineSpacing, instanceOfFontSize } from '#types';
import { CssPropertiesHelper } from './CssPropertiesHelper';
export class CssFontHelper {
    // #region Font
    /**
     ...
     */
    static fillFontProps(style, props, override) {
        const fontWeight = props.fontBold ? 'bold' : undefined;
        const fontSize = CssFontHelper.getFontSizePropsValue(props.fontSize);
        const fontFamily = props.fontAccent ? Theme.FontAccent : undefined;
     ...
     * @returns Свойства CSS по настройкам шрифта в виде TCssProperties
     */
    static getFontPropsBy(size, isBold, isFontAccent) {
        const fontProps = {};
        if (isFontAccent) {
            fontProps.fontFamily = Theme.FontAccent;
        }
     ...
     */
    static getFontSizePropsValue(value) {
     ...
     */
    static getFontSizeInPixels(fontSize) {
     ...
     * @returns Свойства CSS по контейнеру в виде TCssProperties
     */
    static fillTextEffect(style, props, override) {
        const сssProperties = CssFontHelper.getTextEffectPropsBy(props.fontSize, props.textEffect, props.textAlign, props.textLineSpacing);
     ...
     */
    static getTextEffectProps(props) {
        const сssProperties = CssFontHelper.getTextEffectPropsBy(props.fontSize, props.textEffect, props.textAlign, props.textLineSpacing);
        if (Assert.existValue(props.textColor)) {
     ...
     * @returns Свойства CSS по эффектам текста в виде TCssProperties
     */
    static getTextEffectPropsBy(size, effect, textAlign, textLineSpacing) {
        const textProps = {};
        const getSizeShadow = () => {
            if (size) {
                switch (size) {
     ...
     */
    static getLineSpacingPropsValue(value) {
        if (Assert.emptyValue(value))
            return undefined;
...
//# sourceMappingURL=CssFontHelper.js.map