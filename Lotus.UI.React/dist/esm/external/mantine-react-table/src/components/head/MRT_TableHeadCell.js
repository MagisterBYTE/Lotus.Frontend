...
export const MRT_TableHeadCell = ({ columnVirtualizer, header, renderedHeaderIndex = 0, table, ...rest }) => {
    ...
    return (_jsxs(TableTh, { colSpan: header.colSpan, "data-column-pinned": isColumnPinned || undefined, "data-dragging-column": isDraggingColumn || undefined, "data-first-right-pinned": (isColumnPinned === 'right' &&
            ...
            ...parseFromValuesOrFunc(tableCellProps?.style, theme),
                ...
                ? null
                : (tableCellProps.children ?? (_jsxs(Flex, { className: clsx('mrt-table-head-cell-content', classes.content, (columnDefType === 'group' ||
                        tableCellProps?.align === 'center') &&
                        classes['content-center'], tableCellProps?.align === 'right' && classes['content-right'], column.getCanResize() && classes['content-spaced']), children: [_jsxs(Flex, { __vars: {
                                '--mrt-table-head-cell-labels-padding-left': `${headerPL}`,
                            }, className: clsx('mrt-table-head-cell-labels', classes.labels, column.getCanSort() &&
                                columnDefType !== 'group' &&
                                classes['labels-sortable'], tableCellProps?.align === 'right'
                                ? classes['labels-right']
                                : tableCellProps?.align === 'center' &&
                                    classes['labels-center'], columnDefType === 'data' && classes['labels-data']), onClick: column.getToggleSortingHandler(), children: [_jsx(Flex, { className: clsx('mrt-table-head-cell-content-wrapper', classes['content-wrapper'], columnDefType === 'data' &&
                                        classes['content-wrapper-hidden-overflow'], (columnDef.header?.length ?? 0) < 20 &&
                                        classes['content-wrapper-nowrap']), children: headerElement }), column.getCanFilter() &&
                                    (column.getIsFiltered() || showColumnButtons) && (_jsx(MRT_TableHeadCellFilterLabel, { header: header, table: table })), column.getCanSort() &&
                                    (column.getIsSorted() || showColumnButtons) && (_jsx(MRT_TableHeadCellSortLabel, { header: header, table: table }))] }), columnDefType !== 'group' && (_jsxs(Flex, { className: clsx('mrt-table-head-cell-content-actions', classes['content-actions']), children: [showDragHandle && (_jsx(MRT_TableHeadCellGrabHandle, { column: column, table: table, tableHeadCellRef: {
                                        current: tableHeadCellRefs.current[column.id],
                                    } })), columnActionsEnabled && showColumnButtons && (_jsx(MRT_ColumnActionMenu, { header: header, onChange: setIsOpenedColumnActions, opened: isOpenedColumnActions, table: table }))] })), column.getCanResize() && (_jsx(MRT_TableHeadCellResizeHandle, { header: header, table: table }))] }))), columnFilterDisplayMode === 'subheader' && column.getCanFilter() && (_jsx(MRT_TableHeadCellFilterContainer, { header: header, table: table }))] }));
};
//# sourceMappingURL=MRT_TableHeadCell.js.map