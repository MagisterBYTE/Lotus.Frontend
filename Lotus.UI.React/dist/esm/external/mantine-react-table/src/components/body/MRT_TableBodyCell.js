...
export const MRT_TableBodyCell = ({ cell, numRows = 1, renderedColumnIndex = 0, renderedRowIndex = 0, rowRef, table, virtualCell, ...rest }) => {
    ...
    const renderCellContent = () => {
        ...
        if (showSkeletons !== false && (isLoading || showSkeletons)) {
            return _jsx(Skeleton, { height: 20, width: skeletonWidth, ...skeletonProps });
        }
        ...
        if (isCreating || isEditing) {
            return _jsx(MRT_EditCellTextInput, { cell: cell, table: table });
        }
        if (showClickToCopyButton && columnDef.enableClickToCopy !== false) {
            return (_jsx(MRT_CopyButton, { cell: cell, table: table, children: _jsx(MRT_TableBodyCellValue, { ...cellValueProps }) }));
        }
    ...
    return (_jsx(TableTd, { "data-column-pinned": isColumnPinned || undefined, "data-dragging-column": isDraggingColumn || undefined, "data-first-right-pinned": (isColumnPinned === 'right' &&
        ...
        }, className: clsx(classes.root, layoutMode?.startsWith('grid') && classes['root-grid'], virtualCell && classes['root-virtualized'], isEditable &&
            editDisplayMode === 'cell' &&
            classes['root-cursor-pointer'], isEditable &&
            ['cell', 'table'].includes(editDisplayMode ?? '') &&
            columnDefType !== 'display' &&
            classes['root-editable-hover'], columnDefType === 'data' && classes['root-data-col'], density === 'xs' && classes['root-nowrap'], columnDef.enableCellHoverReveal && classes['root-cell-hover-reveal'], tableCellProps?.className), onDoubleClick: handleDoubleClick, onDragEnter: handleDragEnter, onMouseEnter: onMouseEnter, onMouseLeave: onMouseLeave, style: (theme) => ({
            ...widthStyles,
            ...parseFromValuesOrFunc(tableCellProps.style, theme),
        }), children: _jsx(_Fragment, { children: tableCellProps.children ??
                (columnDef.enableCellHoverReveal ? (_jsxs("div", { className: clsx(columnDef.enableCellHoverReveal &&
                        !(isCreating || isEditing) &&
                        classes['cell-hover-reveal'], isCellContentOverflowing && classes['overflowing']), ref: cellHoverRevealDivRef, children: [renderCellContent(), cell.getIsGrouped() && !columnDef.GroupedCell && (_jsxs(_Fragment, { children: [" (", row.subRows?.length, ")"] }))] })) : (_jsxs(_Fragment, { children: [renderCellContent(), cell.getIsGrouped() && !columnDef.GroupedCell && (_jsxs(_Fragment, { children: [" (", row.subRows?.length, ")"] }))] }))) }) }));
};
export const Memo_MRT_TableBodyCell = memo(MRT_TableBodyCell, (prev, next) => next.cell === prev.cell);
//# sourceMappingURL=MRT_TableBodyCell.js.map